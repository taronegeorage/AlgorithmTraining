...
// in-place algorithm, not stable, O(n^2)
template <typename FrwdIt, typename BinaryPred = std::less<typename std::iterator_traits<FrwdIt>::value_type> >
void selection_sort(FrwdIt first, FrwdIt last, BinaryPred comp = BinaryPred()){
    if(std::distance(first, last) <= 1) return;
...
// in-place algorithm, not stable, O(n^（1.3—2）)	
template <typename BidirIt, typename BinaryPred = std::less<typename std::iterator_traits<BidirIt>::value_type> >
